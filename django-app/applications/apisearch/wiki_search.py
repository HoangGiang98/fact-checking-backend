...
class WikiSearch:
  ...
  def parse_results(self, results:list):
    try:
      pages_list:list = [self.wikipedia.page(res["title"]) for res in results]
      return [{ "title" : p.title, "content": p.content, "url": p.url} for p in pages_list]
    except Exception as e:
      print(e)
...
# temp Test
# wk_search = WikiSearch()
# searches = wk_search.search("Coffee")
# for search in searches:
#   print(search["url"])

# print(wk_search.get_url_list())
# parsed_searches = wk_search.parse_results(searches)
