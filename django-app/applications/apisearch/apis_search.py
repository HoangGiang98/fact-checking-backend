...
class ApisSearch:
  ...
  def combined_search(self,search_term:str):
    ...
    self.search_results.update({"results": ApisSearch.filter_results_by_url([*google_result_list, *bing_result_list *wiki_result_list]) })
  ...
  def crawl_preprocess(self, output_dir= "crawled_files", split_by = "passage", split_length= 10):
    crawler = Crawler(
    ...
    indexing_pipeline.add_node(component=self.document_store, name="document_store", inputs=['preprocessor'])
